...
class BST:
    ...
    def _get_height(self, node):
        ...
        #return max - the biggest subtree
        #Use max(x,y) function
        #if node is none or node has 
        
        #Count up nodes in each branch and return the largest one.
        if node == None:
            return 0
        else:
...
def _insert_middle(sorted_list, first, last, bst):
    ...
    if sorted_list == []:
        ...
        if last == 0 or first == (last -1):
            ...
            _insert_middle(sorted_list, first, middle, bst)
            _insert_middle(sorted_list, middle, last, bst)
